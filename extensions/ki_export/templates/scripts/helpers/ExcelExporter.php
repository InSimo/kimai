...
class Kimai_Export_ExcelExporter extends PHPExcel {
	...
	protected function initialize() {
		...
		$this->dateFormat = str_replace('%', '', $this->kga->getDateFormat(1)); // preferring the configurable value over $this->dateformat (hardcoded)
		...
		foreach($this->activeColumns_arr as $x => $columnName) {
			...
			switch($type) {
				...
				case 'money':
					$this->sheet->getStyle($curRange)
								->getNumberFormat()
								->setFormatCode("{$this->kga->getCurrencySign()} " . PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
					break;
				case 'text':
					$this->sheet->getStyle($curRange)
